...
- The primary two-address command now has its first executable runtime slice:
  `TCP-LISTEN:host:port TCP-CONNECT:host:port` binds one TCP listener and opens
  one context-aware destination connection per accepted client through the
  shared owned/drained bridge. Typed parsing completes before runtime dispatch;
  malformed commands and unsupported Tailcat combinations create no listeners
  or outbound connections. Focused command tests cover two sequential clients
  and cancellation; a main-process loopback smoke through the documented HTTP
  backend command returned 200. The two Tailcat command forms remain parsed but
  unwired.
...
- A strict, side-effect-free two-address parser now captures the primary CLI
  ...
  secrets, and unschematized comma options fail closed. Ordinary TCP runtime
  wiring is implemented; Tailcat command wiring is not. Legacy DeleGate
  directives remain a migration adapter.
...
- HTTP CONNECT streams are explicitly owned after net/http hijacking. Parent
  ...
  Darwin/arm64 and Linux/amd64 gate pass. Push-triggered Woodpecker pipeline
  102 passed at exact commit `9651e71`, completing P1-036.
- The geographic bridge now owns each accepted client before starting its
  ...
  Push-triggered Woodpecker pipeline 100 passed all stages at exact commit
  `deaa9a6`.
...
- The compatibility fixture corpus now includes mixed-case option-key
  coverage for HTTPS URL-source legacy `MOUNT` directives.
- The compatibility fixture corpus now includes mixed-case option-key
  coverage for FTP URL-source legacy `MOUNT` directives.
- The compatibility fixture corpus now includes protocol-option case-normalization
...
- The compatibility fixture corpus now includes protocol-option case-normalization
  for CONNECT `MOUNT` metadata with mixed-case `protocol=` and explicit
  `server=default`.
...
- The compatibility fixture corpus now includes mixed-case `MOUNT` option keys in
...
- The compatibility fixture corpus now includes `SERVER=FTP` `CONNECT` metadata
  and selector case-normalization.
- The compatibility fixture corpus now includes `SERVER=GOPHER` `CONNECT` metadata
  and selector case-normalization.
